function DataTable() {
  ...
  const iceGiants = [
    { name: "Uranus", mass: "86.8", diameter: "51,118", density: "1270", gravity: "8.7" },
    { name: "Neptune", mass: "102", diameter: "49,528", density: "1638", gravity: "11.0" },
  ]

  const dwarfPlanets = [
    { name: "Pluto", mass: "0.0146", diameter: "2,377", density: "2095", gravity: "0.7" },
  ]
  ...
  return (
    <section className="table-section">
      ...
      <div className="table-wrapper">
        ...
        <table className="planet-facts-table">
          ...
          <tbody>
            ...
            {terrestrialPlanets.map((planet, index) => (
              <tr key={planet.name}>
                {index === 0 && (
                  <td className="category-cell" colSpan="2" rowSpan={terrestrialPlanets.length}>
                    Terrestrial Planets
                  </td>
                )}
            ...
            {[...gasGiants, ...iceGiants].map((planet, index) => (
              <tr key={planet.name}>
                {index === 0 && (
                  <td className="category-cell main-category" rowSpan={gasGiants.length + iceGiants.length}>
                    Jovian Planets
                  </td>
                )}
                {index === 0 && (
                  <td className="category-cell sub-category" rowSpan={gasGiants.length}>
                    Gas Giants
                  </td>
                )}
                {index === gasGiants.length && (
                  <td className="category-cell sub-category" rowSpan={iceGiants.length}>
                    Ice Giants
                  </td>
                )}
                {renderPlanetCells(planet)}
